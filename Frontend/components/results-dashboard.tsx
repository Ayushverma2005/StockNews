...
export default function ResultsDashboard({ data }: ResultsDashboardProps) {
  ...
  return (
    <div
      className={`mx-auto max-w-[1400px] space-y-8 transition-opacity duration-500 ${isVisible ? "opacity-100" : "opacity-0"}`}
    >
      {/* Company Header */}
      <div className="glass rounded-2xl p-8">
        <h2 className="font-sans text-3xl font-bold text-white">{company.name}</h2>
        <p className="mt-2 font-mono text-xl font-semibold text-blue-400">{company.symbol}</p>
      ...
      {/* Articles Section */}
      <div>
        <h3 className="mb-6 text-2xl font-bold text-white">Analyzed Articles ({article_count} articles)</h3>
